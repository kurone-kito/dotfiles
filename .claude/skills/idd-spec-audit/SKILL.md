...
This bundle's own upstream source repository's `scripts/audit-docs.mjs`
catches byte-level drift there — sync-pair mismatches, budgets,
config-vs-instruction agreement — but not semantic drift: prose that
contradicts a sibling file, leaked session context, passages that are
not completable from a cold read, or wording that stalls an autonomous
step. This installed repository has no equivalent byte-drift tool of
its own. This skill runs the semantic check as N parallel LLM read
...
- **Audit targets** (findings may cite these): `.github/instructions/**/*.md`
  ...
  it from an upstream source
  <!-- dotfiles-divergence: installed-bundle-reference-routing -->
  (for example, this bundle's own upstream source repository,
  [`kurone-kito/idd-skill`](https://github.com/kurone-kito/idd-skill),
  regenerates `.github/instructions/**` from `idd-template/` via
  `audit/sync-manifest.json` — this installed repository has no such
...
- **Out of scope as an audit target / finding source**: this skill's
  ...
  if one exists
  <!-- dotfiles-divergence: installed-bundle-reference-routing -->
  (this installed repository has no such mirror tree today — the
  upstream source repository's own `.claude/**`, by contrast, mirrors a
  canonical source elsewhere); and
