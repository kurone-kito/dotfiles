...
any resulting path that currently exists. If the path is missing from
`PATH`, reconciliation inserts it immediately before `WinGet\Links`.
If the path is already present, reconciliation preserves its position.
