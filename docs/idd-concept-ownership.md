...
| Status / blocker labels (`status:authoring` — draft marker and claim-suppression lock while an issue is held, `status:blocked-by-human`, `status:needs-decision`, `idd:ready`, `triage:{outcome}`) | Human maintainer for the ready label; issue-authoring skill for `status:authoring`; worker session for `status:blocked-by-human` (A1.5 non-autonomous gap, E6 escalation), `status:needs-decision` (any phase, per `idd-overview-appendix.instructions.md`'s Needs-decision claim release paragraph), or the optional diagnostic `triage:` label (A4.5) | Issue-authoring skill removes `status:authoring` only on the user's explicit hold-release request; human maintainer removes `status:blocked-by-human`/`idd:ready` regardless of which actor applied it — resolving that blocker is the human judgment the label exists to enforce; a later worker session removes `status:needs-decision` once a qualifying human decision resolves the hold (`idd-overview-appendix.instructions.md`'s Needs-decision claim release paragraph) | A0/A3/A3.5/A4.5 gates |
...
| `advisory-wait` / `advisory-wait-recovery` / `advisory-reroll` markers | Worker session, E14 or F2/F3 for advisory-wait/recovery; F2-only for advisory-reroll per the advisory-wait protocol | Worker session (minimize a superseded same-PR marker as `OUTDATED`) | AW1-AW3 for advisory-wait/recovery; AW6/F2 for advisory-reroll |
...
| External-check waiver comment (`idd-external-check-waiver`) | Human maintainer only, under the repository's `ciGate.externalCheckWaivers` policy — plus one narrow, documented exception: `github-actions[bot]`, for the `idd-advisory-convergence` selector's own `self-referential-bootstrap-auto` reason token only, when a PR's diff touches that check's own committed trigger-file allowlist (kurone-kito/idd-skill#2657) | Not mutated once posted; superseded by a fresh waiver | F2/F3 CI gate (`waiverEvidence` checks); the bot-posted kind is additionally verified by resolving its `run-id:` field to a `pull_request_target`-triggered run of `idd-advisory-convergence.yml` for the current PR HEAD SHA |
