...
| Merge `master` into the feature branch (post-publication sync) <!-- dotfiles-divergence: master-branch --> | Reversible                | `git merge --abort` / local reset — but only before its own push; once pushed it joins the Push row below and is no longer separately undoable          | E11, E-phase branch-sync check (`idd-review-fix.instructions.md`, `idd-review-triage.instructions.md`) |
