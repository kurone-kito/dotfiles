...
| Critique-pass delegate              | Distributed default: absent (C1 uses the per-agent Critique pass invocation table). This repository now sets `critiqueLoop.delegate` to the PATH-resolved `coderabbit-critique` command with `mode: "combined"`; see [IDD Policy Configuration](idd-policy.md#genuinely-new-in-070).                                                                                                                                                                                                      | Set `critiqueLoop.delegate.command` (and optional `mode`) in `.github/idd/config.json`; see [Critique pass invocation](idd-workflow.md#critique-pass-invocation).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
that required check. Upstream `v0.7.0` moved that refresh to a new
companion `idd-advisory-convergence-comment.yml` workflow: an
IDD-originated comment refreshes the existing HEAD-associated required
run through it instead of the required workflow's own trigger. **This
repository has not yet adopted that companion workflow as of this pin
bump** (evaluated separately, see roadmap #292); its existing single
`idd-advisory-convergence.yml` workflow still handles
`pull_request_review_comment` directly, per the pre-split design both
tags shared through `v0.6.0`, until that adoption decision lands. This
is `idd-advisory-convergence`, not `lint.yml`.
...
workflow's triggers, so after posting a waiver a maintainer must also
...
reflect it. Upstream `v0.7.0` lets an IDD-originated review-thread
comment refresh that same HEAD run via the companion
`idd-advisory-convergence-comment.yml` workflow instead; until this
repository adopts that workflow (see above), a review-thread comment
still refreshes the check via the required workflow's own existing
`pull_request_review_comment` trigger. `workflow_dispatch`
does
...
<https://github.com/kurone-kito/idd-skill/blob/f51a8bb73a47452eff5799e8a27251b660ba4ae0/idd-template/docs/onboarding/policy-decisions.md#idd-label-names>
