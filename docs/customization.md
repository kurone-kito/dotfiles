...
| Worktree guard                      | Advisory only by default — `idd-doctor` warns on a primary-worktree implementation-branch HEAD but nothing blocks the commit, push, or merge                                                                                                                                                                                                                                                          | Upstream ships the opt-in config surface (`worktreeGuard.enabled` in `.github/idd/config.json`), the `.githooks/` hook set, and `idd-doctor`'s enabled-but-inert check together; enabling the flag alone has no effect until a clone also wires `core.hooksPath` (local, per-clone, never committed — every clone must run it once). **This repository has all of it wired**; see [Worktree Guard](idd-policy.md#worktree-guard) for the exact command and verification evidence. Override `worktreeGuard.branchPatterns` to change which branch globs count as implementation branches (default `issue/*`, `roadmap-audit/*`). Absent or `false` keeps the historical advisory-only behavior.                                                               |
