...
- Does NOT contain any
  `<!-- dotfiles-roadmap-id: … -->` marker (the issue
  is not itself a roadmap).
- Does NOT contain any
  `<!-- dotfiles-blocked-by: … -->` marker.
...
**A0-T** (the scoped `dotfiles-roadmap-id` lookup
...
for `dotfiles-roadmap-id` to resolve
`dotfiles-blocked-by` dependency markers; see A2 for
...
or containing an
`<!-- dotfiles-roadmap-id: ... -->` marker is a
...
- All dependency issues are closed or otherwise completed. Check both
  ...
  `<!-- dotfiles-roadmap-id: {roadmap-id} -->`. If that
...
- **Roadmap identity** (`dotfiles-roadmap-id`): placed
...
- **Sequential dependency** (`dotfiles-blocked-by`):
...
**Do not use `dotfiles-blocked-by` to group sub-tasks
