...
3. Run from the **primary worktree**, never from inside the worktree
   ...
   - `git branch -d <branch-name>` (the baseline permission profile
     ...
     before local `master` fast-forwards — run step 4 first, then retry.

4. <!-- dotfiles-divergence: master-branch --> Update the local `master`
   branch (`git fetch origin master && git merge --ff-only
   origin/master`).
