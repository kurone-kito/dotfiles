...
repositories and are loaded automatically by Claude Code from
`~/.claude/CLAUDE.md`.
...
_Note_: the auto-memory system (`~/.claude/projects/…/memory/`) is
a separate per-project mechanism. This file is the static
user-global memory, not auto-memory content.
...
- **Prefer signed commits** by default, and follow this bounded
  ...
  6. **Treat SSH signing as best-effort.** GitHub only marks
     ...
     registered as a _signing_ key on the user's GitHub profile.
  ...
  reviving and may not even exist. Step 1's no-TTY pre-check
  typically fires immediately in this environment class; that still
  satisfies "try the configured signing once" above, not a fourth
  attempt.
